  ...
  return (
    <section id={id} className={`${bg} py-16`}>
      <div className="mx-auto max-w-7xl px-4">
        {title && <h2 className="mb-8 text-3xl font-bold text-text">{title}</h2>}
        {children}
      </div>
    </section>
  );
...
function CardTitle({ children }) {
  return <div className="text-lg font-semibold text-text">{children}</div>;
}
...
function Button({ children, className = "", ...props }) {
  return (
    <button
      {...props}
      className={`inline-flex items-center justify-center gap-2 rounded-2xl border border-border px-4 py-2 text-sm font-medium text-text hover:opacity-90 ${className}`}
    >
      {children}
    </button>
  );
...
function Input(props) {
  return (
    <input
      {...props}
      className={
        "w-full rounded-xl border border-border px-3 py-2 text-sm text-text focus:outline-none focus:ring " +
        (props.className || "")
      }
    />
  );
...
function NavBar() {
  ...
  return (
    <div className="sticky top-0 z-50 w-full border-b border-border bg-white/80 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        <a href="#home" className="inline-flex items-center gap-2">
          <img src="/logo.png" alt="WeGuideAbroad Logo" className="h-10 w-auto" />
        </a>
...
function Hero() {
  const [open, setOpen] = useState(false);
  return (
    <section id="home" className="bg-gradient-to-b from-card to-bg">
      <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 py-16 md:grid-cols-2 md:py-24">
        <div>
          <h1 className="mb-4 text-4xl font-extrabold leading-tight text-text md:text-5xl">
...
function ProgramSearch({ compact = false }) {
  ...
  const results = useMemo(() => {
    return PROGRAMS.filter(
      (p) =>
        (!query ||
          p.program_name.toLowerCase().includes(query.toLowerCase()) ||
    ...
    <div className="min-h-screen scroll-smooth bg-bg text-text">
      ...
      <Section id="programs" title="Explore Programs" bg="bg-white">
        <div className="mb-6 flex items-center justify-between">
          <div className="text-sm text-text/70">{PROGRAMS.length} programs</div>
        </div>
        <ProgramSearch />
      </Section>
      <HowToApply />
